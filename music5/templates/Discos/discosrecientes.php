                ...
                <div class="large-4 columns">
                    <div class="panel marcodisco">
                        ...
                        if(!empty(h($disco->portada))){
                            echo $this->Html->image('../files/discos/portada/' . h($disco['portada_dir']) . '/portrait_' . h($disco['portada']), ['class'=>'portada'], ["alt" => $disco['name']]);
                        }else{
                            h($disco->portada);
                        }
                        ?>
            ...
            <?php foreach ($discos as $disco): ?>
                <tr>
                    ...
                    <td><?
                        if(!empty(h($disco->portada))){
                            echo $this->Html->link(
                                $this->Html->image('../files/discos/portada/' . h($disco['portada_dir']) . '/portrait_' . h($disco['portada']), ['class'=>'portada'], ["alt" => $disco['name']]), ['controller'=>'discos', 'action' => 'view', $disco['id']],
                                ['escape' => false]
                            );
